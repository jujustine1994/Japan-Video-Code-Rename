...
- [ ] **社群資料庫安全性審查**：系統性評估攻擊向量（格式注入、垃圾資料、大量 spam、惡意覆蓋），並針對以下各層加強防禦：
  - **Worker 層**：補強 entries 的 key/value 格式驗證（正規表示式攔截非法番號格式）
  - **GitHub Action 層**：確認現有驗證是否完整，檢查能否被 bypass
  - **Rate limit**：單一 IP 短時間內大量呼叫 Worker 的防護
  - **Issue body 大小**：超大 payload 上傳能否讓 Action 爆炸
  - **資料汙染回復**：git history 為備份，但需確認有快速回復流程
